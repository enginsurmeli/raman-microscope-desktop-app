...
lib.tlccs_init(device_address, 1, 1, byref(ccs_handle))
...
try:
    ...
    integration_time = c_double(
        0.001 * float(input("Please enter the integration time of the spectrometer in ms (allowed range is 0.01 - 60000 ms): ")))
...
for i in range(3648):
    try:
        data_array_absorption[i] = (
            (data_array_ref[i] - data_array_sample[i])/data_array_ref[i])*100
